class PredatorsController < ApplicationController
  ...
  # GET /predators.json
  def index
    logger.info("Current user is " + current_user.to_s)
  ...
  def create
    ...
    respond_to do |format|
      if @predator.save
        ...
        format.json { render action: 'show', status: :created, location: @predator }
      else
  ...
  def destroy
    @predator.destroy
    respond_to do |format|
      format.html { redirect_to predators_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def predator_params
      params.require(:predator).permit(:name, :teeth_size)
    end
end
